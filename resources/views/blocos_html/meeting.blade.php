<header style="border:1px solid #ccc;padding:7px 16px;">
    ...
    <div style="display:flex;">
        ...
        @endphp
        <a rule="button" style="margin-right:30px;font-size:20px;display:none;" id="btn-back-to-meetings"><i
                ...
                <li @if ($message->fk_remetente == Auth::user()->id) class="me" @else class="you" @endif>
                    <div class="entete">
                        @if ($message->fk_remetente == Auth::user()->id)
                            <h3>{{ \Carbon\Carbon::parse($message->created_at)->format('h:s') }},
                                {{ \Carbon\Carbon::parse($message->created_at)->format('d/m/Y') }}</h3>
                            <h2>Eu</h2>
                        @else
    ...
    @if ($permissionToSendMessage)
        <div id="form-to-message" style="padding-left:10px;padding-right:10px;">
            ...
            <button type="button" class="btn btn-primary btn-sm" style="float:right;"
                id="btn-enviar-message">Enviar</button>
        </div>
    @else
        <div class="alert alert-info text-center" role="alert">
            O Envio de Mensagem, é habilitado nos casos:<br>
            Tem Permissão para Ver o pitch da Startup<br>
            É Um Investir a Tentar Comunicar-se Com Outro Investidor
        </div>
    @endif
</section>
