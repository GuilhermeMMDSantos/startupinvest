        ...
        <tbody>
            @foreach ($rodadas as $rodada)
                    ...
                    <td>
                        @if ($tipoUser != 'admin')
                            <a href="{{ route('rodada.page', $rodada->id) }}" rule="button"
                                ...
                                style="height:33px;font-size:14px;border-radius: 30px">Visualizar</a>
                        @else
                            <a href="{{ route('rodada.page.admin', $rodada->id) }}" rule="button"
                                class="btn btn-outline-primary"
                                style="height:33px;font-size:14px;border-radius: 30px">Visualizar</a>
                        @endif
                    </td>
                ...
                <tr class="mobile-table-rodadas" style="font-size:12px;">
                    <td>
                        Ref:{{ $rodada->id }}; @if ($tipoUser != 'startup')
                            startup: {{ $rodada->startup->nome }}
                        @endif
                        <br>
                        valor obtido: {{ number_format($rodada->valor_obtido, 2, ',', '.') }} AOA<br>
                        incio:{{ $rodada->data_inicio }}; fim:@if ($rodada->estado == 'aberta')
                            -
                        @else
                            {{ $rodada->data_fim }}
                        @endif
