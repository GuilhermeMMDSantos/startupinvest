<div style="width: 110px; height: 110px; border: 2px solid #d6d6d6; border-radius: 50%; display: flex; justify-content: center; align-items: center; background: #ffffff;">
...
<div style="width:87%;padding-left:15px;padding-right:5px;">
    <p>
        <span id="nome-startup-intoducao" style="font-size: 1.8rem; font-weight: bold; color: #333; margin-bottom: 5px;">{{$startup->nome}}</span>
        @php
        $codigoStartup = Auth::user()->code_user;
        @endphp
        <input type="text" id="codigo-startup" value="{{Auth::user()->code_user}}" style="display:none;">
        <span style="color: #767d84; font-size: 1rem; margin-left: 10px; margin-right:5px;"><i style="font-size:20px;margin-right:2px;">•</i>{{$startup->setor->nome}}</span>
        <span style="color: #767d84; font-size: 1rem;"><i style="font-size:20px;margin-right:2px;">•</i>{{$startup->fase->nome}}</span>
    </p>
