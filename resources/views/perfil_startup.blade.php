    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $("#experiencia-instituicao-input").keyup(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $("#lista-resultado-busca-instituicao-experiencia").append(response[
                            'html']);
                        if (response['qtd'] > 0)
            ...
            $(document).click(function(elemento) {
                ...
                if (certificadoInputFocus && !certificadoItemClicked && $("#formacao-certificado-input")
                    .val().trim().length > 0) {
                    $("#content-alert-unselected-certificado").html('Não selecionou um certificado');
                    certificadoInputFocus = false;
                }

                if (areaFormacaoInputFocus && !areaFormacaoItemClicked && $("#formacao-area-formacao-input")
                    .val().trim().length > 0) {
                    $("#content-alert-unselected-area-formacao").html(
                        'Não selecionou uma área de formação');
            ...
            $("#btn-salvar-formacao").click(function() {
                ...
                let htmlElement = "<li id='line" + contadorFormacoes + "'>" + certificadoValue + " em " +
                    areaFormacaoValue + "<i class='fa fa-bell dismiss-line' _id='line" + contadorFormacoes +
                    "' style='float:right;' role='button'></i></li>";
            ...
            $("#btn-salvar-experiencia").click(function() {
                ...
                let htmlElement = "<li id='line_experiencia" + contadorExperiencias + "'>" + funcaoValue +
                    " no(a)  " + instituicaoValue +
                    "<i class='fa fa-bell dismiss-line' _id='line_experiencia" + contadorExperiencias +
                    "' style='float:right;' role='button'></i></li>";
            ...
            $("#load_img-membro-equipa-add").change(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        let novaSrc = srcDaImg.substring(0, srcDaImg.indexOf("armazenamento")) +
                            '' + response;
            ...
            $(".btn-remove-img-membro-equipa").click(function() {
                ...
                let srcDaImg = $("#img-membro-equipa-add").attr('src');
                let novaSrc = srcDaImg.substring(0, srcDaImg.indexOf("armazenamento")) +
                    'armazenamento/startups/img/membros/img_standard_membro_equipa.png';
                $("#img-membro-equipa-add").attr('src', novaSrc);
            ...
            $("#btn-adicionar-membro-equipa").click(function() {
                ...
                for (let iterador = 0; iterador <= contadorFormacoes; iterador++) {
                    ...
                    if (formacoes['line' + iterador] != undefined) {
                        ...
                        formacaoString = formacoes['line' + iterador]['certificado'] + '|' + formacoes[
                            'line' + iterador]['areaformacao'];
                ...
                for (let iterador = 0; iterador <= contadorExperiencias; iterador++) {
                    if (experiencias['line_experiencia' + iterador] != undefined) {
                        experienciaString = experiencias['line_experiencia' + iterador]['funcao'] + '|' +
                            experiencias['line_experiencia' + iterador]['idFuncao'] + '|' + experiencias[
                                'line_experiencia' + iterador]['instituicao'] + '|' + experiencias[
                                'line_experiencia' + iterador]['idInstituicao'];
                ...
                if (escolheuCargo == false) {
                    $("#content-alert-unselected-cargo-membro").html(
                        "Deve escolher pelo menos um(1) cargo. Somente deve existir um(1) colaborador a exercer um determinado cargo."
                    );
                    return false;
                }
            ...
            function resetarFormularioAdicionarMembro() {
                ...
                let novoValorAtributoSrcDaImagemMembro = valorAtualAtributoSrcDaImagemMembro.substring(0,
                        valorAtualAtributoSrcDaImagemMembro.indexOf("armazenamento")) +
                    'armazenamento/startups/img/membros/img_standard_membro_equipa.png';
            ...
            function show_valor_a_acrescer(valor) {
                ...
                let valor_acrescer = (valor * 25 / 100).toFixed(2) +
                    ''; // o paypal desconsta 25 em cada valor transacionado
            ...
            function check_empty_field(myForm) {
                ...
                if (!myForm.get('termino')) {
                    ...
                    find_error = 1;
                } else if (new Date(new Date(myForm.get('termino')).toDateString()) - new Date(new Date()
                        .toDateString()) < 604800000) {
                    $("#alert-data-angariacao").html(
                        "O tempo para angariação deve ser de (1) uma semana no mínimo");
                    $("#termino-oferta").addClass("is-invalid");
                    find_error = 1;
                } else if (new Date(new Date(myForm.get('termino')).toDateString()) - new Date(new Date()
                        .toDateString()) > 16070400000) //milisegundos = 6meses com 31 dias cada
                {
                    $("#alert-data-angariacao").html(
                        "O tempo para angariação deve ser de (6) seis meses no máximo");
                    $("#termino-oferta").addClass("is-invalid");
                    find_error = 1;
                }
